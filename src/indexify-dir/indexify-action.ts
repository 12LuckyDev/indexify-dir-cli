...
export const indexifyAction = (fileExtension: string, workingPath: string | null, options: IndexifyOptions): void => {
  ...
  const extenstionsWhiteList = allExtensions ? null : (options.extensions ?? [fileExtension]);

  if (verbose && extenstionsWhiteList !== null) {
    console.info(
      pc.cyan(`Files with ${extenstionsWhiteList.join(' ')} extensions will be included in ${fileName} file.`),
    );
  }
